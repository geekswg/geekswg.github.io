(function() {
  ...
  // 获取参数
  const snowCount = urlParams.get('snowCount') || 120;
  ...
  class Snowflakes {
    ...
    createSnowflake(randomY = false) {
      ...
      const y = randomY ? this.random(-100, window.innerHeight * 0.8) : -size;
      ...
      const opacity = this.random(this.config.opacity * 0.7, this.config.opacity);
    ...
    startAnimation() {
      ...
      const animate = () => {
        ...
        for (let i = this.snowflakes.length - 1; i >= 0; i--) {
          ...
          // 轻微透明度变化
          if (Math.random() < 0.01) {
            flake.element.style.opacity = (flake.opacity * (0.8 + Math.random() * 0.4)).toString();
          }
    ...
    resetSnowflake(flake) {
      ...
      // 重新随机化属性
      flake.speed = this.random(this.config.minSpeed, this.config.maxSpeed);
      ...
      flake.rotation = 0;
      flake.opacity = this.random(this.config.opacity * 0.7, this.config.opacity);
      
      // 随机选择新的雪花符号
      flake.element.textContent = this.config.symbols[
    ...
    bindEvents() {
      ...
      window.addEventListener('resize', () => {
        this.snowflakes.forEach(flake => {
          if (flake.x > window.innerWidth) flake.x = window.innerWidth * 0.5;
          if (flake.y > window.innerHeight) flake.y = window.innerHeight * 0.5;
        });
      });
    }
